...
const statusLabel: Record<string, string> = {
  current: "เริ่มทำได้ตอนนี้", in_progress: "กำลังพัฒนา", locked: "ยังรอทักษะก่อนหน้า", flexible: "เลือกทำเมื่อไรก็ได้",
};
...
export default function CoursePage() {
  ...
  useEffect(() => {
    ...
    api.resource(resourceId, userId, session.readTarget())
      .then(setCourse)
      .catch((e: unknown) => setError(e instanceof Error ? e.message : "โหลดรายละเอียด Course ไม่สำเร็จ"));
  }, [resourceId]);
    ...
    <main className="flex-1 min-w-0">
      <header className="border-b border-border-low bg-surface-bg sticky top-0 z-20">
        <div className="max-w-container-max mx-auto px-margin-mobile md:px-gutter h-16 flex items-center gap-3">
          <button onClick={() => router.back()} className="w-9 h-9 grid place-items-center rounded-lg hover:bg-surface-muted" aria-label="กลับไปยัง Roadmap"><Icon>arrow_back</Icon></button>
          <div className="min-w-0"><p className="text-xs text-text-subtle">Roadmap / Course detail</p><p className="text-sm font-semibold truncate">{course?.roadmap_context?.title_en ?? "SIIT Roadmap"}</p></div>
        </div>
      </header>

      <div className="max-w-5xl mx-auto px-margin-mobile md:px-gutter py-stack-lg pb-32">
        {error && <InlineNotice tone="error">{error} <button onClick={() => router.push("/roadmap")} className="underline font-semibold">กลับไป Roadmap</button></InlineNotice>}
        {!course && !error && <p className="py-24 text-center text-text-subtle">กำลังโหลด Course และผลต่อ roadmap…</p>}
        {course && <>
          <section className="rounded-2xl bg-surface-container-low border border-border-low p-5 md:p-8 relative overflow-hidden">
            ...
            <div className="relative max-w-3xl">
              ...
              <p className="mt-3 text-text-subtle">{course.provider ?? "ผู้ให้บริการไม่ระบุ"} · ประมาณ {course.est_hours} ชั่วโมง · {course.cost_baht === 0 ? "ไม่มีค่าใช้จ่าย" : `${course.cost_baht.toLocaleString()} บาท`}</p>
              {course.description && <p className="mt-5 text-on-surface-variant leading-relaxed">{course.description}</p>}
              {course.url && <a href={course.url} target="_blank" rel="noreferrer" className="mt-6 inline-flex items-center gap-2 bg-primary text-on-primary px-4 py-2.5 rounded-lg font-semibold hover:bg-primary-container"><Icon>open_in_new</Icon>เปิด Course ของผู้ให้บริการ</a>}
            </div>
          </section>
          ...
          <section className="mt-6 grid grid-cols-1 lg:grid-cols-[1.1fr_.9fr] gap-6">
            <article className="bg-surface-bg border border-border-low rounded-xl p-5 md:p-6">
              <div className="flex items-center gap-2"><Icon className="text-primary">route</Icon><h2 className="text-xl font-semibold">Course นี้มีผลต่อ Roadmap อย่างไร</h2></div>
              ...
              <div className="mt-5 space-y-3">
                {course.teaches.map((skill) => <div key={skill.skill_id} className="p-4 rounded-lg bg-surface-muted border border-border-low">
                  <div className="flex flex-col sm:flex-row gap-2 sm:items-start sm:justify-between"><div><p className="font-semibold">{skill.name_th}</p><p className="mt-1 text-xs text-text-subtle">เมื่อทำ Course นี้ครบ ระบบคาดว่าคุณจะไปถึงระดับ {skill.reaches_level}</p></div>{skill.roadmap_status && <span className="w-fit text-xs px-2 py-1 rounded-full bg-primary-fixed text-primary">{statusLabel[skill.roadmap_status]}</span>}</div>
                  <div className="mt-3 flex flex-wrap gap-2 text-xs text-text-subtle">{skill.current_level != null && <span className="bg-surface-bg px-2 py-1 rounded">ตอนนี้ระดับ {skill.current_level}</span>}{skill.roadmap_target_level != null && <span className="bg-surface-bg px-2 py-1 rounded">เป้าหมายระดับ {skill.roadmap_target_level}</span>}{skill.unlock_count > 0 && <span className="bg-secondary-container text-secondary px-2 py-1 rounded">ปลดล็อกต่อได้ {skill.unlock_count} ทักษะ</span>}</div>
                </div>)}
              </div>
            </article>

            <article className="bg-surface-bg border border-border-low rounded-xl p-5 md:p-6">
              <div className="flex items-center gap-2"><Icon className="text-primary">checklist</Icon><h2 className="text-xl font-semibold">ตัวอย่างการลงมือทำ</h2></div>
              ...
              <div className="mt-6 p-4 rounded-lg bg-tertiary-fixed border border-tertiary-fixed-dim"><p className="text-xs font-semibold text-tertiary">หลักฐานเมื่อทำเสร็จ</p><p className="mt-1 text-sm text-on-surface-variant">{course.proof_of_done}</p></div>
              {course.data_status === "placeholder" && <p className="mt-4 text-xs text-text-subtle">ข้อมูล Course นี้เป็นข้อมูลตัวอย่าง ต้องตรวจรายละเอียดจริงกับผู้ให้บริการก่อนสมัคร</p>}
            </article>
          </section>
        </>}
      </div>
